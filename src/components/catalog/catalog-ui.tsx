...
export function Notice({
  ...
  return (
    <div
      ...
      className={`${styles.notice} ${kind === "error" ? styles.error : kind === "success" ? styles.success : ""}`}
    >
      {children}
      {retry && (
        <div>
          <button type="button" onClick={retry} className={styles.button}>
            Повторить загрузку
          </button>
        </div>
      )}
    ...
    <article className={styles.card}>
      <div className={styles.badges}>
        <span className={styles.badge}>
          {task.confirmedIndustry || "Не указано"}
        </span>
        <span
          className={`${styles.badge} ${styles[task.confirmedScore.level]}`}
        >
          {LEVELS[task.confirmedScore.level].label}
        </span>
      </div>
...
export function TaskFields({ content }: { content: TaskContent }) {
  return (
    <dl>
      {FIELD_SECTIONS.flatMap((section) => section.fields).map((field) => (
        <div key={field.key} className={styles.detail}>
          <dt>{field.label}</dt>
          <dd className={content[field.key].trim() ? "" : styles.muted}>
            {content[field.key].trim() || "Не указано"}
          </dd>
        </div>
      ))}
    </dl>
  ...
  return (
    <div>
      <div className={styles.badges}>
        ...
        <span className={`${styles.badge} ${styles[score.level]}`}>
          {LEVELS[score.level].label} · {LEVELS[score.level].range}
        </span>
      </div>
      <p className={styles.intro}>
        Официальный рейтинг подтверждённой карточки.
      </p>
      {score.groups.map((group) => (
        <div className={styles.group} key={group.id}>
          ...
          {group.missing.length > 0 && (
            <small>
              Не хватает: {group.missing.map((item) => item.label).join("; ")}
            </small>
          )}
        </div>
      ))}
...
export function TeamProfile({ team }: { team: Team }) {
  return (
    <div className={styles.profile}>
      ...
      <small>{team.xp} XP за подтверждённые этапы</small>
