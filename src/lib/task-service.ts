...
export class TaskService {
  ...
  async teamProposals(teamId: string) {
    z.uuid().parse(teamId);
    if (!(await this.repo.listTeams()).some((team) => team.id === teamId))
      throw notFound();
    return (await this.repo.listProposals()).filter((p) => p.teamId === teamId);
  }
  async seedDemo() {
    const data = demoDataset();
    const teams = await this.repo.listTeams();
    if (
      data.teams.some((team) => !teams.some((stored) => stored.id === team.id))
    )
      throw new ApiError(
        503,
        "DEMO_TEAMS_MISSING",
        "Сначала примените supabase/schema.sql: нужны пять демокоманд.",
      );
    // Fixed IDs make retries safe. Never overwrite user edits to examples.
    for (const task of data.tasks) {
      await this.repo.createTask(this.owner, task);
      await this.owned(task.id);
    }
    for (const proposal of data.proposals) {
      if (!(await this.repo.getProposal(proposal.id)))
        await this.repo.createProposal(proposal);
    }
