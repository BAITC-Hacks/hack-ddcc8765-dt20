...
describe("participant 3 collaboration adapter", () => {
  ...
  it("seeds five published levels and five distinct drafts without hiding weak tasks", async () => {
    ...
    expect(new Set(catalog.map((task) => task.confirmedScore.level))).toEqual(new Set(["draft", "working", "ready", "priority"]));
    expect(catalog.find((task) => task.confirmedScore.total === 0)).toBeTruthy();
    ...
    const proposal = await collaboration.submitProposal(weak.id, {
      teamId: (await collaboration.teams())[0].id, idea: "Проверить процесс", plan: "Собрать макет", deadline: "Неделя", prototypeUrl: null,
    });
    expect(proposal.status).toBe("pending");
    expect((await collaboration.businessProposals(weak.id)).some((item) => item.id === proposal.id)).toBe(true);
    const draft = (await collaboration.businessTasks()).find((task) => !task.publishedAt)!;
    await expect(collaboration.publicTask(draft.id)).rejects.toThrow("не опубликована");
  });

  it("accepts two teams independently and awards XP once for a confirmed result", async () => {
    const { collaboration } = await import("../src/lib/collaboration-client");
    const catalog = await collaboration.catalog();
    const bakery = catalog.find((task) => task.confirmedContent.title.includes("пекарни"))!;
    const proposals = await collaboration.businessProposals(bakery.id);
    expect(proposals).toHaveLength(2);
    await collaboration.decide(proposals[1].id, "accepted");
    expect((await collaboration.businessProposals(bakery.id)).filter((item) => item.status === "accepted")).toHaveLength(2);
    await collaboration.confirmStage(proposals[0].id);
    await collaboration.confirmStage(proposals[0].id);
    expect((await collaboration.teams()).find((team) => team.id === proposals[0].teamId)?.xp).toBe(10);
    await expect(collaboration.confirmStage(proposals[1].id)).rejects.toThrow("дождитесь результата");
  });
  ...
  it("keeps builder publication in the same local catalog and protects confirmed content", async () => {
    ...
    const task = newTask("30000000-0000-4000-8000-000000000001");
    task.draft.title = "Новая слабая задача";
    await gateway.create(task);
    await gateway.confirm(task);
    await gateway.publish(task);
    const before = await collaboration.publicTask(task.id);
    const current = await gateway.get(task.id);
    current.draft.need = "Новое неподтверждённое требование";
    await gateway.save(current);
    expect((await collaboration.publicTask(task.id)).confirmedContent.need).toBe("");
    await gateway.confirm(current);
    expect((await collaboration.publicTask(task.id)).confirmedContent.need).toBe("Новое неподтверждённое требование");
  ...
  it("uses public HTTP routes and never treats server errors as local success", async () => {
    ...
    const requests = vi.fn()
      .mockResolvedValueOnce(Response.json({ error: "База недоступна", code: "STORAGE_UNAVAILABLE" }, { status: 503 }))
    ...
    await expect(collaboration.publicTask("task-id")).rejects.toThrow("неожиданный ответ");
